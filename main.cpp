...
int main(int argc, char const *argv[])
{
	...
	// for(trace_id = 0; trace_id < cfg.no_trace; trace_id ++){
		...
		sprintf(fname, "result/%s/%df_%dx%d_trace_%d.txt", metadata.video.name.c_str(), metadata.vp.No_face, metadata.vp.No_tile_h, metadata.vp.No_tile_v, metadata.trace.TRACE_ID);
		fout = open_file(fname);
		fprintf(fout, "BW\t");
		for(method_id = 0; method_id < cfg.no_method; method_id++){
...
void write_result(AdaptLogic adaptLogic, int N, int BW){
	...
	FILE* fVPSNR;
	// generate log files
	sprintf(fname, "result/%s/log_tile_ver_TRACE_%d_BW_%d_METHOD_%d_VP_EST_METHOD_%d_INTER_%d_BUFF_%d_%df_%dx%d.txt",adaptLogic.metadata.video.name.c_str(), adaptLogic.metadata.trace.TRACE_ID, BW, adaptLogic.metadata.adapt.TILE_SELECT_METHOD, adaptLogic.metadata.adapt.VP_EST_METHOD,adaptLogic.metadata.video.INTERVAL, adaptLogic.metadata.video.BUFF, adaptLogic.metadata.vp.No_face, adaptLogic.metadata.vp.No_tile_h, adaptLogic.metadata.vp.No_tile_v);
	printf("#%s\n", fname);
